...
##  Modify Tp Trigger Mails Only For Prod Server            -Arshad Taqvi  05/07/2025
####################################################################################
####################################################################################
...
{ echo "$($fileprefix),$t3,$t2,$hn,FAILURE,$SQLSERVER,$ports " | Out-File $outputfile1 -Append
  ##Report Failure in SQLQUERY FILE AS WELL
  $t2=100000
  echo "$($fileprefix),$t3,$t2,$hn,$db3,$SQLSERVER " | Out-File $outputfile -Append
...
#Report Prod All Day
##DONT SEND MAIL FOR NON PROD ANYMORE JUST TRIGGER ALERT FOR PROD-ARSHAD TAQVI
#if (($TD -ne "Sunday" ) -and ($TD -ne "Saturday") -and ($SQLServer -notlike '%PRD%'))
#{
#if (($THH -gt 7) -and ($THH -lt 18) -and ($SQLServer -notlike '%PRD%'))
#{
# Send-MailMessage @sendMailMessageBeacon
#}
#}
